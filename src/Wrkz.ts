 ...
 */
export class Wrkz extends HTTPClient implements WrkzTypes.IWrkz {
    /**
    ...
    public async transactionsStatus (
        transactions: string[]
    ): Promise<WrkzTypes.ITransactionsStatus> {
    ...
    public async randomIndexes (
        amounts: number[],
        count = 3
    ): Promise<WrkzTypes.IRandomOutput[]> {
    ...
    public async indexes (
        startHeight: number,
        endHeight: number
    ): Promise<WrkzTypes.ITransactionIndexes[]> {
        return this.get('indexes/' + startHeight + '/' + endHeight)
            .then((result: WrkzTypes.ITransactionIndexes[]) =>
    ...
    public async sync (
        ...
        count = 100): Promise<WrkzTypes.ISync> {
        ...
        response.blocks = response.blocks.map((block: WrkzTypes.ISyncBlock) => {
            if (block.coinbaseTX) {
    ...
    public async rawSync (
        ...
        timestamp = 0,
        skipCoinbaseTransactions = false,
        count = 100): Promise<WrkzTypes.IRawSync> {
