...
/**
 * A class interface that allows for easy interaction with Legacy Wrkz
 * THIS OBJECT IS DEPRECATED AND SUBJECT TO REMOVAL WITH LITTLE NOTICE
 */
export class LegacyWrkz extends HTTPClient implements WrkzTypes.IWrkz {
    /**
     * Retrieves the node fee information
     */
    public async fee (): Promise<WrkzTypes.IFee> {
        const response: LegacyWrkzTypes.IFeeResponse = await this.get('fee');
     ...
     */
    public async height (): Promise<WrkzTypes.IHeight> {
        const response: LegacyWrkzTypes.IHeightResponse = await this.get('height');
     ...
     */
    public async info (): Promise<WrkzTypes.IInfo> {
        const response: LegacyWrkzTypes.IInfoResponse = await this.get('info');

        const parse = (elem: string): WrkzTypes.IVersion => {
     ...
     */
    public async peers (): Promise<WrkzTypes.IPeers> {
        const response: LegacyWrkzTypes.IPeersResponse = await this.get('peers');
     ...
     */
    public async block (block: string | number): Promise<WrkzTypes.IBlock> {
        ...
        const response: LegacyWrkzTypes.IBlockSummary = await this.rpcPost('f_block_json', { hash: block })
            .then(response => response.block);

        const result: WrkzTypes.IBlock = {
     ...
     */
    public async lastBlock (): Promise<WrkzTypes.IBlock> {
     ...
     */
    public async blockHeaders (height: number): Promise<WrkzTypes.IBlock[]> {
        const response = await this._blockShortHeaders(height);

        const promises: Promise<WrkzTypes.IBlock>[] = response.map(block => this.block(block.hash));
     ...
     * @param block the block height or hash
     */
    public async rawBlock (block: string | number): Promise<WrkzTypes.IRawBlock> {
        let header: LegacyWrkzTypes.IBlockHeader;
    ...
    public async blockTemplate (
        address: string,
        reserveSize = 8
    ): Promise<WrkzTypes.IBlockTemplate> {
        const response: LegacyWrkzTypes.IBlockTemplate = await this.rpcPost(
     ...
     */
    public async transaction (hash: string): Promise<WrkzTypes.ITransaction> {
        const response = await this._transaction(hash);

        const toOutput = (output: LegacyWrkzTypes.IOutputKey): WrkzTypes.ITransactionOutputKey => {
            return {
        ...
        const toCoinbaseInput =
            (input: LegacyWrkzTypes.IInputCoinbase): WrkzTypes.ITransactionInputCoinbase => {
                return {
        ...
        const toKeyInput =
            (input: LegacyWrkzTypes.IInputKey): WrkzTypes.ITransactionInputKey => {
                return {
        ...
        for (const input of response.tx.vin) {
            switch (input.type) {
                case 'ff':
                    coinbase = true;
                    inputs.push(toCoinbaseInput(input as LegacyWrkzTypes.IInputCoinbase));
                    break;
                case '02':
                    inputs.push(toKeyInput(input as LegacyWrkzTypes.IInputKey));
                    break;
            }
        }
        ...
        return {
            ...
            prefix: {
                ...
                inputs: (coinbase)
                    ? inputs as WrkzTypes.ITransactionInputCoinbase[]
                    : inputs as WrkzTypes.ITransactionInputKey[],
     ...
     * in the memory pool
     */
    public async transactionPool (): Promise<WrkzTypes.TransactionSummary[]> {
    ...
    public async transactionPoolChanges (
        ...
        transactions: string[] = []
    ): Promise<WrkzTypes.ITransactionPoolDelta> {
        const response = await this._poolChanges(lastKnownBlock, transactions);

        const result: WrkzTypes.ITransactionPoolDelta = {
            added: [],
        ...
        for (const added of response.addedTxs) {
            ...
            const addCoinbase = (input: LegacyWrkzTypes.IInputCoinbase) =>
                tx.inputs.push(new CoinbaseInput(input.value.height));

            const addKeyInput = (input: LegacyWrkzTypes.IInputKey) =>
            ...
            for (const input of added.prefix.vin) {
                switch (input.type) {
                    case 'ff':
                        addCoinbase(input as LegacyWrkzTypes.IInputCoinbase);
                        break;
                    case '02':
                        addKeyInput(input as LegacyWrkzTypes.IInputKey);
                        break;
                }
            }
     ...
     */
    public async transactionsStatus (transactions: string[]): Promise<WrkzTypes.ITransactionsStatus> {
     ...
     */
    public async randomIndexes (amounts: number[], count: number): Promise<WrkzTypes.IRandomOutput[]> {
        const response = await this._randomOutputs(amounts, count);

        const results: WrkzTypes.IRandomOutput[] = [];
     ...
     */
    public async indexes (startHeight: number, endHeight: number): Promise<WrkzTypes.ITransactionIndexes[]> {
        ...
        const results: WrkzTypes.ITransactionIndexes[] = [];
    ...
    public async rawSync (
        ...
        skipCoinbaseTransactions = false,
        count = 100
    ): Promise<WrkzTypes.IRawSync> {
        const response = await this._rawBlocks(height, timestamp, checkpoints, skipCoinbaseTransactions, count);

        const toRawBlock = (rawBlock: LegacyWrkzTypes.IRawBlock): WrkzTypes.IRawBlock => {
            return {
                blob: rawBlock.block,
        ...
        const result: WrkzTypes.IRawSync = {
     ...
     */
    private async _lastBlockHeader (): Promise<LegacyWrkzTypes.IBlockHeader> {
     ...
     */
    private async _blockHeaderByHash (hash: string): Promise<LegacyWrkzTypes.IBlockHeader> {
     ...
     */
    private async _blockHeaderByHeight (height: number): Promise<LegacyWrkzTypes.IBlockHeader> {
     ...
     */
    private async _blockShortHeaders (height: number): Promise<LegacyWrkzTypes.IBlockShortHeader[]> {
    ...
    private async _globalIndexesForRange (
        startHeight: number,
        endHeight: number
    ): Promise<LegacyWrkzTypes.IGlobalIndexesResponse[]> {
    ...
    private async _poolChanges (
    ...
    ): Promise<LegacyWrkzTypes.IPoolChanges> {
        const body: any = {
        ...
        const tmp: LegacyWrkzTypes.IBlockLiteTransaction[] = [];
    ...
    private async _randomOutputs (
        amounts: number[],
        mixin = 1
    ): Promise<LegacyWrkzTypes.IRandomOutputsResponse> {
    ...
    private async _rawBlocks (
        ...
        skipCoinbaseTransactions = false,
        blockCount = 100
    ): Promise<LegacyWrkzTypes.IRawBlocksResponse> {
    ...
    private async _sendRawTransaction (
        transaction: string
    ): Promise<LegacyWrkzTypes.ISendRawTransactionResponse> {
     ...
     */
    private async _transaction (hash: string): Promise<LegacyWrkzTypes.ITransactionResponse> {
     ...
     */
    private async _transactionPool (): Promise<LegacyWrkzTypes.ITransactionSummary[]> {
    ...
    ): Promise<LegacyWrkzTypes.ITransactionStatusResponse> {
    ...
    private async _walletSyncData (
        ...
        skipCoinbaseTransactions = false,
        blockCount = 100
    ): Promise<LegacyWrkzTypes.IWalletSyncData> {
